...
def test1():
    ...
    node1 = LimNode("node1", c=1.0, g=0.1, h0=1.0, dq=dq)
    branch1 = LimBranch("branch1", l=0.1, r=0.1, e0=1.0, dq=dq)
...
def test3_eigen():
    ...
    sys.run(1.000001, ode=False, qss=True, verbose=True, ode_method="Radau",
            optimize_dq=0, chk_ss_delay=False)

    sys.print_natural_frequencies()
...
def pendulum():
    ...
    pendulum = Pendulum("pendulum", r=r, l=l, theta0=theta0, omega0=omega0,
                        dq_omega=dq_omega, dq_theta=dq_theta)
    ...
    for atom in sys.state_atoms:
        ...
        pth = f"C:\\Temp\\qdl\\pendulum\\{atom.device.name}_{atom.name}"
...
if __name__ == "__main__":
    ...
    #test3_eigen()   # ship sys reduced # 1 eigen value dump
    #test4()
    #pendulum()
    #pendulum2()  # coupled pendulums
    #test7()
    #test8()
    #test9()   # dc motor sys
